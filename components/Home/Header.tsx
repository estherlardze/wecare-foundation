import React from "react";

const Header = () => {
  return (
    <div className="bg-hero bg-cover bg-no-repeat bg-center mt-[60px] w-full h-[100vh]">
      <div className="bg-black/50 h-[100vh] text-white gap-4 w-full">
        <div className="w-[86%] mx-[7%] flex flex-col items-center justify-center h-[100vh]">
          <h1 className="font-bold text-xl sm:text-2xl mb-4">
            We are creating a brighter tomorrow
          </h1>
          <p className="text-2xl sm:text-3xl md:text-4xl font-bold my-6">
            Make someones life by giving of yours
          </p>
          <button className="px-4 py-2 bg-red-700 rounded-full hover:bg-yellow-600 transition-all">
            Learn More
          </button>
        </div>
      </div>
    </div>
  );
};

export default Header;
